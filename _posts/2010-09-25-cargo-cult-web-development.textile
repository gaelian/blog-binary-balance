...
It couldn't be, I thought to myself. But it was. This guy had secured his admin UI _using JavaScript_. When the supplied username and password are not found in the database the PHP code outputs inline JavaScript that redirects the user back to the login page with the query string containing 'pid=2' which incidentally just signals to the login page that it should show an 'Invalid username and password' error message.
